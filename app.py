...
4. **Respond with Relevant Card Groups**: Based on the tool's output, suggest categories and several keywords that align with the user's needs. Include examples or specific card groups that might help them with particular activities or communication goals. Always display card images as thumbnails {{thumbnail}} (there may be files with name.png.png or name.svg.png) under link {{path}} and card to contain only image, category and keyword.
